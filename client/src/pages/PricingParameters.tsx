    ...
    <DashboardLayout>
      <div className="space-y-6">
        <div>
          ...
          <p className="text-gray-600">Configure os parâmetros para cálculo de propostas</p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure os custos mensais da empresa</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm font-medium">Custo Fixo Mensal (R$)</label>
                <Input
                  type="number"
                  value={formData.monthlyFixedCost}
                  onChange={(e) => setFormData({ ...formData, monthlyFixedCost: parseFloat(e.target.value) || 0 })}
                  placeholder="Ex: 5000"
                />
              </div>

              <div>
                <label className="text-sm font-medium">Custo de Mão de Obra Mensal (R$)</label>
                <Input
                  type="number"
              ...
              <div>
                <label className="text-sm font-medium">Horas Produtivas por Mês</label>
                <Input
                  type="number"
                  value={formData.productiveHoursPerMonth}
                  onChange={(e) => setFormData({ ...formData, productiveHoursPerMonth: parseFloat(e.target.value) || 160 })}
                  placeholder="Ex: 160"
                />
              </div>
              ...
              <div className="bg-blue-50 p-4 rounded">
                ...
                <p className="text-2xl font-bold text-blue-600">R$ {calculateHourlyRate()}</p>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Configure regime tributário e ajustes</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
                <label className="text-sm font-medium">Regime Tributário Padrão</label>
                <select
                  value={formData.defaultTaxRegime}
                  onChange={(e) => setFormData({ ...formData, defaultTaxRegime: e.target.value as "MEI" | "SN" | "LP" | "autonomous" })}
              ...
              <div>
                <label className="text-sm font-medium">Ajuste de Risco (%)</label>
                <Input
                  type="number"
                  value={formData.riskAdjustment}
                  onChange={(e) => setFormData({ ...formData, riskAdjustment: parseFloat(e.target.value) || 0 })}
                  placeholder="Ex: 10"
                />
                <p className="text-xs text-gray-500 mt-1">Percentual adicional para projetos com maior risco</p>
              </div>

              <div>
                <label className="text-sm font-medium">Ajuste de Senioridade (%)</label>
                <Input
                  type="number"
                  value={formData.seniorityAdjustment}
                  onChange={(e) => setFormData({ ...formData, seniorityAdjustment: parseFloat(e.target.value) || 0 })}
                  placeholder="Ex: 15"
                />
                <p className="text-xs text-gray-500 mt-1">Percentual adicional para consultores sênior</p>
              </div>
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-2">
            <div className="flex justify-between">
              ...
              <span className="font-medium">R$ {formData.monthlyFixedCost.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Custo de MO Mensal:</span>
              <span className="font-medium">R$ {formData.laborCost.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-600">Custo Total Mensal:</span>
              <span className="font-medium">R$ {(formData.monthlyFixedCost + formData.laborCost).toFixed(2)}</span>
            </div>
            <div className="flex justify-between border-t pt-2">
              <span className="text-gray-600">Valor da Hora Técnica:</span>
              <span className="font-bold text-lg">R$ {calculateHourlyRate()}</span>
            </div>
          </CardContent>
        </Card>
